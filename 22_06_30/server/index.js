...
var app = express();   // express 모듈 안에 있는 express class 불러오는 과정
...
app.get("/",function(req, res){  // localhost:3000/ 로 호출했을때
    ...
    res.render("index.ejs");    // 이 화면(index.ejs)을 브라우저에 덮어씌워주기 
});
...
var port = 3000;
app.listen(port, function(){   // 웹 서버가 실행이 됐을때 시작 콘솔에 프린트
    console.log("웹 서버 시작");
})
